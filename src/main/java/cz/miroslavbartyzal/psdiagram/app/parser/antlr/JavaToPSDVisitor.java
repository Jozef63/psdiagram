...
public class JavaToPSDVisitor extends Java8BaseVisitor<Flowchart<LayoutSegment, LayoutElement>> {
	...
	public Flowchart<LayoutSegment, LayoutElement> visitArrayInitializer(Java8Parser.ArrayInitializerContext ctx) {
		String arrayInitializer = ctx.getText();
		if (arrayInitializer.startsWith("{") && arrayInitializer.endsWith("}")) {
			arrayInitializer = arrayInitializer.replace("{", "[");
			arrayInitializer = arrayInitializer.replace("}", "]");
		}
		cz.miroslavbartyzal.psdiagram.app.gui.symbolFunctionForms.Process.generateValues(lastElement.getSymbol(),
				lastElement.getSymbol().getCommands().get("var"), arrayInitializer);
...
//		System.out.println("visitChildren(ctx): " + visitChildren(ctx));
	...
	public Flowchart<LayoutSegment, LayoutElement> visitBlockStatements(Java8Parser.BlockStatementsContext ctx) {
		if (ctx.getChildCount()>0) {
			for(ParseTree child : ctx.children) {
				System.out.println("BlockStatementsChild: " + child.getText() + " is instance of: " + child.getClass().getSimpleName());
			} 
		}
		System.out.println(ctx.getClass().getSimpleName()+" : "+ctx.getText());visitChildren(ctx);return flowchart;
	}
			...
			for(ParseTree child : ctx.children) {
				...
				if (child instanceof ExpressionContext) {
					...
					if (expressionText.contains("<")) {
						...
						if (expressionText.contains("=")) {
							to = varAfterOperand ? expressionText.substring(0, expressionText.indexOf("<")) : expressionText.substring(expressionText.indexOf("=")+1);
							Integer incremented = ((Integer.valueOf(to)) +1);
					...
					if (expressionText.contains(">")) {
						...
						if (expressionText.contains("=")) {
							to = varAfterOperand ? expressionText.substring(0, expressionText.indexOf(">")) : expressionText.substring(expressionText.indexOf("=")+1);
							Integer decremented = ((Integer.valueOf(to)) -1);
	...
	public Flowchart<LayoutSegment, LayoutElement> visitEnhancedForStatementNoShortIf(Java8Parser.EnhancedForStatementNoShortIfContext ctx) {
		if(ctx.getChildCount()>0) {
			for(ParseTree child : ctx.children) {
				System.out.println("enhancedForChild: " + child.getText() + ", is instance of: " + child.getClass().getSimpleName());
			}
		}
		visitChildren(ctx);return flowchart;
	}
